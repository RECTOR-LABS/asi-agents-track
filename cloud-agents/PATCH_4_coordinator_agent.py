"""
MediChain AI - Coordinator Agent (Cloud Deployment - PATCHED v4)
Simplified inter-agent message handling using direct agent handlers.
...
# Chat protocol for ASI:One (this MUST be a protocol)
...
async def handle_chat_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    ctx.logger.info(f"📨 [PATCH v4] Received chat message")
...
# ============================================================================
# INTER-AGENT MESSAGE HANDLERS (Direct agent handlers)
# ============================================================================
...
async def handle_agent_acknowledgement(ctx: Context, sender: str, msg: AgentAcknowledgement):
    ...
    ctx.logger.info(f"✅ Acknowledgement from {msg.agent_name}")
...
async def handle_diagnostic_request(ctx: Context, sender: str, msg: DiagnosticRequest):
    ...
    ctx.logger.info(f"📥 Diagnostic request from {sender}")
    ctx.logger.info(f"   Session: {msg.session_id}")
...
async def startup(ctx: Context):
    ctx.logger.info("=" * 60)
    ctx.logger.info("MediChain AI Coordinator (PATCH v4)")
    ctx.logger.info("=" * 60)
    ctx.logger.info(f"Agent address: {agent.address}")
    ctx.logger.info(f"Chat Protocol: Enabled (ASI:One)")
    ctx.logger.info(f"Inter-Agent: Direct agent handlers (no protocol)")
    ctx.logger.info("=" * 60)


# Include chat protocol ONLY (inter-agent uses direct handlers)
