"""
MediChain AI - Coordinator Agent (Cloud Deployment - PATCHED v3)
Fixed inter-agent message handling with published protocol manifest.
...
# Initialize inter-agent protocol - MUST match Patient Intake's protocol name
...
async def handle_chat_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    for item in msg.content:
        if isinstance(item, StartSessionContent):
            ...
            ctx.logger.info(f"📝 Text from user: {item.text[:50]}...")
            session.add_message("user", item.text)

            # Route to Patient Intake Agent
...
async def handle_chat_ack(ctx: Context, sender: str, msg: ChatAcknowledgement):
    ...
    ctx.logger.info(f"✅ Received acknowledgement from user")
...
async def handle_symptom_analysis_response(ctx: Context, sender: str, msg: SymptomAnalysisResponseMsg):
    ...
    ctx.logger.info(f"📥 Received symptom analysis from {sender}")
...
async def startup(ctx: Context):
    ...
    ctx.logger.info("=" * 60)
    ctx.logger.info("MediChain AI Coordinator (Cloud - PATCHED v3)")
    ctx.logger.info("=" * 60)
    ctx.logger.info(f"Agent address: {agent.address}")
    ctx.logger.info(f"Chat Protocol: Enabled (ASI:One)")
    ctx.logger.info(f"Inter-Agent Protocol: MediChainProtocol (published)")
    ctx.logger.info("=" * 60)
