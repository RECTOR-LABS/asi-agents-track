"""
MediChain AI - Patient Intake Agent (Cloud Deployment - PATCHED v3)
...
class IntakeTextMessage(Model):
    """Message from coordinator with symptom text"""
    text: str
    session_id: str
...
# Create inter-agent protocol - MUST match Coordinator's protocol name
...
async def handle_intake_message(ctx: Context, sender: str, msg: IntakeTextMessage):
    ...
    # If no symptoms after max clarifications
    if not symptoms:
...
async def startup(ctx: Context):
    ...
    ctx.logger.info("=" * 60)
    ctx.logger.info("MediChain AI - Patient Intake Agent (Cloud - PATCHED v3)")
    ctx.logger.info("=" * 60)
    ctx.logger.info(f"Agent address: {agent.address}")
    ctx.logger.info(f"Protocol: MediChainProtocol (published)")
...
# Include the inter-agent protocol WITH published manifest
