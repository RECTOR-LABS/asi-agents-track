"""
MediChain AI - Patient Intake Agent (Cloud Deployment - PATCHED v6)
Proper protocol registration with 'replies' parameter (from official docs).
...
# ============================================================================
# AGENT & PROTOCOL INITIALIZATION
# ============================================================================

agent = Agent()

# Create protocol with explicit reply types (FROM OFFICIAL DOCS!)
intake_proto = Protocol()


# ============================================================================
# MESSAGE HANDLER - WITH 'replies' PARAMETER (CRITICAL!)
# ============================================================================

@intake_proto.on_message(model=IntakeTextMessage, replies={AgentAcknowledgement, DiagnosticRequest})
async def handle_intake_message(ctx: Context, sender: str, msg: IntakeTextMessage):
    """Process patient symptom descriptions"""
    ctx.logger.info(f"✅ [PATCH v6 - WITH REPLIES!] Received intake message")
...
async def startup(ctx: Context):
    ctx.logger.info("=" * 60)
    ctx.logger.info("MediChain AI - Patient Intake Agent (PATCH v6)")
    ctx.logger.info("=" * 60)
    ctx.logger.info(f"Agent address: {agent.address}")
    ctx.logger.info(f"Message handling: Protocol with 'replies' parameter")
    ctx.logger.info(f"Ready to extract patient symptoms")
    ctx.logger.info("=" * 60)


# Include protocol (with published manifest)
